  ...
  try {
    ...
    if (customers.data.length > 0) {
      ...
      if (subscriptions.data.length > 0) {
        ...
        // Instead of blocking, allow them to continue - they might be upgrading
      }
    } else {
      logStep("No existing Stripe customer found, will create during checkout");
    }

    const origin = req.headers.get("origin") || "https://materialink.io";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: cancelUrl || `${origin}/subscriptions?canceled=true`,
      metadata: {
        user_id: user.id,
      },
    });
